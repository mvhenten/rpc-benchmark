var main = (function () {
	 ...
	 */
	Namespace.prototype.add = function add(object$$1) {

	    if (!(object$$1 instanceof field && object$$1.extend !== undefined || object$$1 instanceof Type$1 || object$$1 instanceof Enum || object$$1 instanceof Service$1 || object$$1 instanceof Namespace))
	        throw TypeError("object must be a valid nested object");

	    if (!this.nested)
	        this.nested = {};
	    else {
	        var prev = this.get(object$$1.name);
	        if (prev) {
	            if (prev instanceof Namespace && object$$1 instanceof Namespace && !(prev instanceof Type$1 || prev instanceof Service$1)) {
	                // replace plain namespace but keep existing nested elements and options
	                var nested = prev.nestedArray;
	                for (var i = 0; i < nested.length; ++i)
	                    object$$1.add(nested[i]);
	                this.remove(prev);
	                if (!this.nested)
	                    this.nested = {};
	                object$$1.setOptions(prev.options, true);

	            } else
	                throw Error("duplicate name '" + object$$1.name + "' in " + this);
	        }
	    }
	    this.nested[object$$1.name] = object$$1;
	    object$$1.onAdd(this);
	...
	OneOf.prototype.onAdd = function onAdd(parent) {
	    ...
	    for (var i = 0; i < this.oneof.length; ++i) {
	        var field$$1 = parent.get(this.oneof[i]);
	        if (field$$1 && !field$$1.partOf) {
	            field$$1.partOf = self;
	            self.fieldsArray.push(field$$1);
	        }
	    }
	...
	OneOf.prototype.onRemove = function onRemove(parent) {
	    for (var i = 0, field$$1; i < this.fieldsArray.length; ++i)
	        if ((field$$1 = this.fieldsArray[i]).parent)
	            field$$1.parent.remove(field$$1);
	...
	Service$2.prototype.create = function create(rpcImpl, requestDelimited, responseDelimited) {
	    ...
	    for (var i = 0, method$$1; i < /* initializes */ this.methodsArray.length; ++i) {
	        var methodName = util_1.lcFirst((method$$1 = this._methodsArray[i]).resolve().name).replace(/[^$\w_]/g, "");
	    ...
	    fieldsById: {
	        get: function() {
	            ...
	            for (var names = Object.keys(this.fields), i = 0; i < names.length; ++i) {
	                var field$$1 = this.fields[names[i]],
	                    id = field$$1.id;
	...
	Type$2.generateConstructor = function generateConstructor(mtype) {
	    ...
	    for (var i = 0, field$$1; i < mtype.fieldsArray.length; ++i)
	        if ((field$$1 = mtype._fieldsArray[i]).map) gen
	            ("this%s={}", util_1.safeProp(field$$1.name));
	        else if (field$$1.repeated) gen
	            ("this%s=[]", util_1.safeProp(field$$1.name));
	    return gen
	 ...
	 * @returns {Writer} writer
	 */
	Type$2.prototype.encode = function encode_setup(message$$1, writer$$1) {
	    return this.setup().encode(message$$1, writer$$1); // overrides this method
	};
	 ...
	 * @returns {Writer} writer
	 */
	Type$2.prototype.encodeDelimited = function encodeDelimited(message$$1, writer$$1) {
	    return this.encode(message$$1, writer$$1 && writer$$1.len ? writer$$1.fork() : writer$$1).ldelim();
	};
	 ...
	 */
	Type$2.prototype.decode = function decode_setup(reader$$1, length) {
	    return this.setup().decode(reader$$1, length); // overrides this method
	};
	 ...
	 */
	Type$2.prototype.verify = function verify_setup(message$$1) {
	    return this.setup().verify(message$$1); // overrides this method
	};
	 ...
	 * @returns {Object.<string,*>} Plain object
	 */
	Type$2.prototype.toObject = function toObject(message$$1, options) {
	    return this.setup().toObject(message$$1, options);
	};
	...
	Root.prototype.resolveAll = function resolveAll() {
	    if (this.deferred.length)
	        throw Error("unresolvable extensions: " + this.deferred.map(function(field$$1) {
	            return "'extend " + field$$1.extend + "' in " + field$$1.parent.fullName;
	        }).join(", "));
	    return namespace.prototype.resolveAll.call(this);
	};
	 ...
	 * @see {@link Root#load}
	 */
	function load(filename, root$$1, callback) {
	    if (typeof root$$1 === "function") {
	        callback = root$$1;
	        root$$1 = new protobuf.Root();
	    } else if (!root$$1)
	        root$$1 = new protobuf.Root();
	    return root$$1.load(filename, callback);
	}
	 ...
	 */
	function loadSync(filename, root$$1) {
	    if (!root$$1)
	        root$$1 = new protobuf.Root();
	    return root$$1.loadSync(filename);
	}
	...
	function parse$1(source, root$$1, options) {
	    ...
	    function parseField(parent, rule, extend) {
	        ...
	        /* istanbul ignore if */
	        if (!typeRefRe.test(type$$1))
	            throw illegal(type$$1, "type");
	        ...
	        var field$$1 = new field(name, parseId(next()), type$$1, rule, extend);
	        ...
	        if (!isProto3 && field$$1.repeated && (types_1.packed[type$$1] !== undefined || types_1.basic[type$$1] === undefined))
	    ...
	    function parseMapField(parent) {
	        ...
	        skip("=");
	        var field$$1 = new mapfield(applyCase(name), parseId(next()), keyType, valueType);
	        ifBlock(field$$1, function parseMapField_block(token) {
	    ...
	    function parseMethod(parent, token) {
	        ...
	        var method$$1 = new method(name, type$$1, requestType, responseType, requestStream, responseStream);
	...
	getCjsExportFromNamespace(schema$1);
